...
class DualDDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        # box, cls = d_cat.split((self.reg_max * 4, self.nc), 1)
        # dbox = (dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides)
        # box2, cls2 = torch.cat([di.view(shape[0], self.no, -1) for di in d2], 2).split((self.reg_max * 4, self.nc), 1)
        # dbox2 = (dist2bbox(self.dfl2(box2), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides)
        # y = [torch.cat((dbox, cls.sigmoid()), 1), torch.cat((dbox2, cls2.sigmoid()), 1)]

        d_cat = torch.cat([di.view(shape[0], self.no, -1) for di in d1], 2)
        d_cat2 = torch.cat([di.view(shape[0], self.no, -1) for di in d2], 2)
        if self.export and self.format in {
            "saved_model",
            "pb",
            "tflite",
            ...
            box = d_cat[:, : self.reg_max * 4]
            cls = d_cat[:, self.reg_max * 4 :]
            box2 = d_cat2[:, : self.reg_max * 4]
            cls2 = d_cat2[:, self.reg_max * 4 :]
        else:
            box, cls = d_cat.split((self.reg_max * 4, self.nc), 1)  # !!MODIFIED!!
            box2, cls2 = d_cat2.split((self.reg_max * 4, self.nc), 1)  # !!MODIFIED!!
...
class BaseModel(nn.Module):
    ...
    def fuse(self, **kwargs):  # fuse model Conv2d() + BatchNorm2d() layers
